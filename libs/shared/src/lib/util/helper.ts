...
export function parseInt2(
  inputInteger: number | string | null | undefined
): number | undefined {
  let parsedInteger: number | undefined;
  if (
    inputInteger === null ||
    inputInteger === undefined ||
    isNaN(inputInteger as number)
  ) {
...
export function parseFloat2(
  inputFloat: number | string | null | undefined
): number | undefined {
  ...
  if (x === y) {
    ...
    for (const prop in x) {
      if (prop in y) {
        ...
        if (!internalDeepEqual((x as any)[prop], (y as any)[prop]))
          return false;
      } else {
        return false;
      }
...
function hasOwnProperty<X extends Record<string, unknown>, Y extends keyof X>(
  obj: X,
  prop: Y
...
export function replacementValue<X extends Partial<X>, K extends keyof X>(
  values: X,
  self: X,
  prop: K
): (X & Record<K, unknown>)[K] {
...
export function getTotal<T extends Array<X>, X>(
  values: T,
  target: NumberKey<X>
) {
  return values.reduce((p, c) => p + (c[target] as number), 0);
}

export function getAverage<T extends Array<X>, X>(
  values: T,
  target: NumberKey<X>
) {
...
export function lessThanXHourAgo(date: Date, hours = 1) {
